...
Modeling viewport framing update:

- `modeler/lib/BaseViewer.js` now exposes `fitViewport({ padding, maxZoom })`, which computes the current diagram bounds from diagram-js elements and relation waypoints, then sets the canvas viewbox with padding.
- `app/src/@types/modeler.d.ts` now includes the `fitViewport` API so React code can call it safely.
- `app/src/components/ModelerState.tsx` now calls `fitViewport` after initial imports, saved graph opens, uploads, example opens, and other `import.done` events.
- `app/src/App.tsx` now gives the `<main>` landmark a fixed full-height layout so the modeler canvas can correctly fill the viewport after the accessibility landmark change.
...
- `.gitignore` now ignores `.playwright-mcp/`, which is generated by browser automation during UI debugging and should not be committed.
